...
```{r}
max100 <- c() #création d'un vecteur vide
for(i in 1:100){
 max100[i] = max(runif(10,-1,1))
}
hist(max100,breaks=100)
densitermax100 <- density(max100)
lines(densitermax100,lwd=2,col="blue") #superposition de la densité de la loi
...
E[\hat{\theta}] = \theta
$$
On se sert de la linéarité de l'espérance : 
$$
...
L'espérance de la loi est 2. 
En prenant un échantillon 
$$
(X_1,X_2,...,X_n)
$$
Le paramètre d'intérêt est donc l'estimateur de cette loi: 
$$
...
$$
Il va donc falloir faire un échantillon d'un échantillon. 
...
La courbe est symétrique ,la médiane d'une loi de cauchy est \theta. D'après RStudio, quand la position n'est pas défini celle-ci est mise à 0. Par conséquent nous devons vérifier si la médiane semble proche de 0.
