...
i <- seq(1,10)
contour(i,i,outer(i,i,logLGamma))
```
...
```{r}
x0 <- c(1,1)
# la fonction optim minimise il faut donc faire -1 pour la maximiser
...
#histogramme pour le vectreur alpha et le vecteur beta

hist(alphaVect)
hist(betaVect)
```

On remarque les valerus de alpha et beta sont plus importante vers 2.5 et 1.5 ce qui sont les résultats que l'on souhaite originellement 
...
L'augmentation de la taille de l'échantillon permet de se rapprocher de la bonne valeur $\theta$.
