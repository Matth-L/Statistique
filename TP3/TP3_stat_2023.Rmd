...
l'échantillon.Cependant cette méthode n'est pas très précise pour de petits échantillons.
...
```
LA plupart du temps, le sommet de la fonction se trouve a x=p.
...
Il faut utiliser la logvraisemblance car le log transforme les multiplications en addition donc cela permet de réduire l'instabilité.
...
\end{array}\right)
$$
Avec $\psi$ la fonction digamma dérivé de la fonction gamma.
...
```{r question 14}
hessien <- optim(par = x0, fn = logLGammaV2,echantillon=rgamma(15,2.5,1.5), method = "L-BFGS-B", control = list(fnscale = -1), hessian = TRUE) 
